"""
Example 10: Double DQN (DDQN) with CartPole (no gym)

Focus:
- Double DQN target to reduce overestimation
...
def train_ddqn(
    ...
    seed: int,
    log_interval: int,
) -> tuple[DDQNAgent, List[float], List[float]]:
    ...
    return agent, reward_history, epsilon_history


def evaluate(agent: DDQNAgent, episodes: int = 10, seed: int = 0) -> float:
    env = CartPoleEnv(max_steps=500, seed=seed)
...
def plot_training_curves(
    ...
    axes[0].plot(episodes, reward_history, color="#0072B2", linewidth=1.2, alpha=0.6)
    window = 50
    if len(reward_history) >= window:
        moving_avg = np.convolve(reward_history, np.ones(window) / window, mode="valid")
        axes[0].plot(range(window, len(reward_history) + 1), moving_avg, color="#D55E00", linewidth=2.0)
    axes[0].set_title("Episode Rewards")
    axes[0].set_xlabel("Episode")
    axes[0].set_ylabel("Reward")
    axes[0].grid(alpha=0.25)

    axes[1].plot(episodes, epsilon_history, color="#009E73", linewidth=1.6)
    ...
    fig.tight_layout()
    out_file = save_dir / "ddqn_training.png"
    fig.savefig(out_file, dpi=150)
    plt.close(fig)
    print(f"Saved: {out_file}")


def rollout_trajectory(env: CartPoleEnv, agent: DDQNAgent, num_steps: int = 500) -> dict:
    state, _ = env.reset()
    traj = {
        "x": [state[0]],
        "x_dot": [state[1]],
...
def plot_trajectory(save_dir: Path, traj: dict, dt: float = 0.02) -> None:
    ...
    fig.tight_layout()
    out_file = save_dir / "ddqn_trajectory.png"
    fig.savefig(out_file, dpi=150)
    plt.close(fig)
    print(f"Saved: {out_file}")
...
def create_cartpole_animation(save_dir: Path, traj: dict) -> None:
    ...
    gif_path = save_dir / "ddqn_cartpole.gif"
    imageio.mimsave(gif_path, frames, duration=0.05)
    print(f"Saved: {gif_path}")


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Double DQN for CartPole (no gym)")
...
def main() -> None:
    ...
    print("Double DQN (DDQN) for CartPole, no gym")
    save_dir = Path("outputs")
    agent, rewards, eps_history = train_ddqn(
    ...
    plot_training_curves(save_dir, rewards, eps_history)
