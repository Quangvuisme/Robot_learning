"""
Example 09: DQN with Epsilon-Greedy + Replay Buffer (CartPole, no gym)
...
def plot_training_curves(
    ...
    fig.tight_layout()
    out_file = save_dir / "dqn_eps_replay_training.png"
...
def plot_trajectory(save_dir: Path, traj: dict, dt: float = 0.02) -> None:
    ...
    fig.tight_layout()
    out_file = save_dir / "dqn_eps_replay_trajectory.png"
    fig.savefig(out_file, dpi=150)
    plt.close(fig)
    print(f"Saved: {out_file}")
...
def create_cartpole_animation(save_dir: Path, traj: dict) -> None:
    ...
    gif_path = save_dir / "dqn_eps_replay.gif"
    imageio.mimsave(gif_path, frames, duration=0.05)
    print(f"Saved: {gif_path}")


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="DQN with epsilon-greedy + replay buffer (CartPole, no gym)")
...
def main() -> None:
    ...
    print("DQN with epsilon-greedy + replay buffer (no gym)")
    save_dir = Path("outputs")
    agent, rewards, eps_history = train_dqn(
